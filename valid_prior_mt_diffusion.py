...
plt.title('MSE per frame, action:{}'.format(text[:-4]))
plt.tight_layout()
save_path = os.path.join(save_dir, "prior_diffusion_pred_{}_steps_{}.pdf".format(pred_step, text[:-4]))
plt.savefig(save_path)
plt.close()
print('保存多步预测MSE变化至 {}'.format(save_path))
...
for idx in range(0,output_rot_mat.shape[1],3):
    plt.plot(all_times[200:], output_rot_mat[200:,idx,0].cpu(), label='joint {}'.format(idx))

plt.xlabel('time (seconds)')
plt.ylabel('angle')
save_path_motion = os.path.join(save_dir, "prior_diffusion_pred_{}_steps_{}_motion.pdf".format(pred_step, text[:-4]))
...
# 噪声水平控制
v_tau *= model.noise_scale
# 开始去噪
Tau = model.diffusion_schedule.Tau # cosine选50步可以吗

tau = Tau
# v_tau = model.denoise(v_tau, history_poses, context, tau, sampled_z=None)

B, T, N, d_in = history_poses.shape

beta_tau = model.diffusion_schedule.betas[tau]
alpha_tau = model.diffusion_schedule.alphas[tau]
alpha_bar_tau = model.diffusion_schedule.alpha_bars[tau]

mask = torch.zeros(1,T).to(v_tau.device) # 历史数据全部能看见

# tau: (B, T_q)
tau_tensor = torch.full((B,1),tau,dtype=torch.long,device=v_tau.device)
tau_embed = model.tau_embed(tau_tensor) # (B, T_q, D)

epsilon_scaled_theta = model.decode(v_tau, history_poses, tau_embed, context, mask)
# get_loss中，学到的就是放大版的噪声，要把它缩回原来的尺度
epsilon_theta = epsilon_scaled_theta / model.noise_scale
# (B, 1, N, d_in)
mu_tau = 1/alpha_tau.sqrt() * (v_tau - beta_tau/(1-alpha_bar_tau).sqrt() * epsilon_theta)
