...
params_to_train = model.get_pretrain_params()
# 加载模型优化器
...
def train_one_epoch(model, train_loader, optimizer, device='cuda'):
    ...
    for batch in progress_bar:
        ...
        optimizer.zero_grad()
        loss = model.get_pretrain_loss(rot_mats, 0.3)
        loss.backward()
...
def get_model_size_in_gb(model):
    ...
    total_gb = total_params / (1024 ** 2)  # 转换为 GB Byte->kB->MB->GB
    return total_gb
# 报告模型大小
...
for epoch in range(len(train_loss_all),len(train_loss_all)+num_epoch):
    ...
    plt.savefig(os.path.join(save_dir,'loss_{}_pretrain.pdf'.format(ckpt_name[:-4])))
    plt.close()
