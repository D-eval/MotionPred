...
def norm_6d(v):
    ...
    for i in range(dim//3 - 1):
        v[...,3*i:3*i+3] /= (v[...,3*i:3*i+3]**2).sum().sqrt()
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, noised_velocity, history_pose, tau_emb, context=None, mask=None, fix_lenght=True):
        # noised_velocity: (B, T, N, d): velocity = (inputs[:,1:T+1] - inputs[:,:T])
        # noised_velocity (tau) = (alpha_bar[tau]).sqrt() * velocity + (1-alpha_bar[tau]).sqrt() * noise
        # 每个 noised_velocity 只能看到自己之前的 history_pose
        # history_pose: (B, T, N, d)
        # tau_emb: 理论上可以是frame_wise的(B, T, D) 或者batch_wise的 (B, D) 它们得到相同的语义
        # (B, T_q, D)
        # 但是不能是joint_wise的(B,T,N,D)，因为noised_velocity要经过spacial attention，要保持噪声的一致性。
        # 我们这里选择frame_wise的 (B, T, D)
        # context: (B, C, D)
        # return: noise_pred: (B, T, N, d)
        B, T, N, d = history_pose.shape
        noised_velocity = noised_velocity.permute(2,0,1,3) # (N,B,T_q,d)
        embed = []
        for joint_idx in range(self.num_joints):
            joint_rep = self.embeddings[joint_idx](noised_velocity[joint_idx]) # (B,T,D)
            embed += [joint_rep]
        
        eps_v = torch.stack(embed) # (N,B,T_q,D)
        eps_v = eps_v.permute(1,2,0,3) # (B,T_q,N,D)

        if self.abs_pos_encoding:
            eps_v += self.pos_encoding[:, :T]
            
        gate = torch.sigmoid(self.tau_gate_proj(tau_emb)) # (B, T_q, D)
        eps_v = eps_v + gate[:, :, None, :] * tau_emb[:, :, None, :] # woc,以后不用unsqueeze了
        
        eps_v = self.input_dropout(eps_v) # (B,T_q,N,D)
        ...
        for i in range(self.num_layers):
            eps_v, block1, block2 = self.para_transformer_layers[i](eps_v, history_pose, look_ahead_mask, context)
        ...
        if not self.use_6d_outputs:
            # (N,B,T_q,D)
            eps_v = eps_v.permute(2,0,1,3)
            output = []
            for joint_idx in range(self.num_joints):
                joint_output = self.output_linears[joint_idx](eps_v[joint_idx])
                output += [joint_output]
            ...
            output = []
            for joint_idx in range(self.num_joints):
                joint_output = self.output_linears[joint_idx](eps_v[joint_idx]) # (B, T, d)
                output += [joint_output]
            
            # n_joints = eps_v.shape[0]
            # batch_size = eps_v.shape[1]
            # seq_len = eps_v.shape[2]
# 
        ...
        if self.residual_velocity: # 这里指的是扩散时间tau velocity，不是现实时间t velocity
            final_output += noised_velocity.permute(1,2,0,3)
...
class TransformerDiffusion(nn.Module):
    def __init__(self,num_joints,joint_size,
        ...
        self.noise_scale = nn.Parameter(torch.tensor(0.02)) # std 0.02 -> std 1
    ...
    def decode(self, noised_velocity, history_pose, tau_emb, context, mask):
        # inputs: (B, T, N, d_in)
        # context: (B, C, D2)
        outputs, _ = self.decoder(noised_velocity, history_pose, tau_emb, context, mask)
        return outputs
    
    def forward(self, long_seq, inputs, tau=None):
        # long_seq: (B, T_l, N, d_in)
        # inputs: (B, T+1, N, d_in)
        # tau: int 扩散模型时间
        # outputs: (B, T, N, d_in) 每个向后预测一步
        B, T_add_one, N, d_in = inputs.shape
        T = T_add_one - 1
        Tau = self.diffusion_schedule.Tau
        context = self.encode(long_seq) # (B, C, D2)
        tau = sample_tau(B, T, Tau, inputs.device) if tau is None else tau # (B, T)
        loss = self.get_loss(inputs, context, tau)
        return loss
    ...
    def tau_embed(self, tau):
        ...
        tau_embedding = self.tau_embedding[tau.view(-1)].reshape(B, T, -1) # (B, T, D)
        return tau_embedding

    def get_loss(self, inputs, context, tau):
        # inputs: (B, T+1, N, d_in)
        # context: (B, C, D2)
        # tau: 扩散模型时间 (B, T) {0,1,2,3,...,Tau-1}
    ...
    def denoise(self, v_tau, history_poses, context, tau, sampled_z=None):
        # 确保模型处于eval模式
        # v_tau: (B, 1, N, d_in) 仅预测下一时刻
        # tau: int 扩散模型时间
        # history_pose: (B, T, N, d_in)
        # context: (B, C, D2)
        # sampled_z: (B, 1, N, d_in) 正态分布采样, 将diffusion引向不同的生成结果
        # return v_tau_minus_one: (B, 1, N, d_in) 去除一步噪声
        ...
        mask = torch.zeros(1,T).to(v_tau.device) # 历史数据全部能看见
        
        # tau: (B, T_q)
        tau_tensor = torch.full((B,1),tau,dtype=torch.long,device=v_tau.device)
        tau_embed = self.tau_embed(tau_tensor) # (B, T_q, D)
        
        epsilon_scaled_theta = self.decode(v_tau, history_poses, tau_embed, context, mask)
        # get_loss中，学到的就是放大版的噪声，要把它缩回原来的尺度
        # epsilon_theta = epsilon_scaled_theta / self.noise_scale
        # (B, 1, N, d_in)
        # 加噪用的是 scaled noise,降噪也用这个
        mu_tau = 1/alpha_tau.sqrt() * (v_tau - beta_tau/(1-alpha_bar_tau).sqrt() * epsilon_scaled_theta)
        if tau == 1:
            return mu_tau
        ...
        sampled_z = torch.randn_like(v_tau) if sampled_z is None else sampled_z
        sampled_z *= self.noise_scale # 统一尺度
        v_tau_minus_one = mu_tau + sigma_tau * sampled_z
        return v_tau_minus_one

    def sample(self, history_poses, context, sampled_z=None):
        # history_poses: (B, T, N, d_in)
        # context: (B, C, D2)
        B, T, N, d_in = history_poses.shape
        # 初始噪声
        v_tau = torch.randn(B, 1, N, d_in).to(history_poses.device)
        # 噪声水平控制
        v_tau *= self.noise_scale
        # 开始去噪
        Tau = self.diffusion_schedule.Tau # cosine选50步可以吗
        for tau in range(Tau,0,-1): # [Tau, Tau-1,..., 1]
            v_tau = self.denoise(v_tau, history_poses, context, tau, sampled_z=sampled_z)
        new_pose = history_poses[:, -1, :, :] + v_tau.squeeze(1) # (B, N, d_in)
        return new_pose[:,None,:,:] # (B, 1, N, d_in)
    ...
    def denoise_each(self, v_tau, history_poses, context, tau, sampled_z=None):
        # 没什么意义，只是用于评估模型
        # v_tau: (B, T, N, d_in) 每人都要预测下一时刻
        # tau: int 扩散模型时间
        # history_pose: (B, T, N, d_in)
        # context: (B, C, D2)
        # sampled_z: (B, 1, N, d_in) 正态分布采样, 将diffusion引向不同的生成结果
        # return v_tau_minus_one: (B, 1, N, d_in) 去除一步噪声
        B, T, N, d_in = history_poses.shape
        assert v_tau.shape[1]==T,"denoise_each每个时间都预测，如果只想预测下一步，用denoise"
        ...
        # tau: (B, T_q)
        tau_tensor = torch.full((B,T),tau,dtype=torch.long,device=v_tau.device)
        tau_embed = self.tau_embed(tau_tensor) # (B, T_q, D)
        
        epsilon_scaled_theta = self.decode(v_tau, history_poses, tau_embed, context, mask=None) # mask传入None默认为look_ahead
        # get_loss中，学到的就是放大版的噪声，要把它缩回原来的尺度
        # epsilon_theta = epsilon_scaled_theta / self.noise_scale
        # (B, T, N, d_in)
        mu_tau = 1/alpha_tau.sqrt() * (v_tau - beta_tau/(1-alpha_bar_tau).sqrt() * epsilon_scaled_theta)
        ...
        sigma_tau = var_tau.sqrt()
        sampled_z = torch.randn_like(v_tau) if sampled_z is None else sampled_z
        sampled_z *= self.noise_scale #
        v_tau_minus_one = mu_tau + sigma_tau * sampled_z
        return v_tau_minus_one

    def sample_each(self, history_poses, context):
        # history_poses: (B, T, N, d_in)
        # context: (B, C, D2)
        B, T, N, d_in = history_poses.shape
        # 初始噪声
        v_tau = torch.randn(B, T, N, d_in).to(history_poses.device)
        # 噪声水平控制
        v_tau *= self.noise_scale
        # 开始去噪
        Tau = self.diffusion_schedule.Tau # cosine选50步可以吗
        for tau in range(Tau,0,-1): # [Tau, Tau-1,..., 1]
            v_tau = self.denoise_each(v_tau, history_poses, context, tau, sampled_z=0) # 预测不需要丰富
        new_poses = history_poses + v_tau # (B, T, N, d_in)
        return new_poses # (B, T, N, d_in)
