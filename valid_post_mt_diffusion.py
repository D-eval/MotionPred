...
import math
import numpy as np
from motionTransformerDiffusion import TransformerDiffusion, compute_rotation_matrix_from_ortho6d, norm_6d
import json
...
'''
# valid_all()
all_frame_loss = get_multi_time_pred_loss(0)
'''
...
with torch.no_grad():
    ...
    for t in range(pred_step):
        ...
        history_poses = torch.cat([history_poses, new_pose], dim=1)
    # history_poses (1, T, N, 6)
    loss = ((history_poses - rot_mat)**2).mean()
'''
...
valid_set.processor.write_bvh(output_rot_mat,filename='posterior_diffusion_frame_{}_motion_{}.bvh'.format(pred_step,text[:-4]))
valid_set.processor.write_bvh(rot_mat_flatten,filename='posterior_diffusion_frame_{}_motion_{}_ground_truth.bvh'.format(pred_step,text[:-4]))
...
for idx in range(0,output_rot_mat.shape[1]):
